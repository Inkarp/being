...
export default function ProductsSidebar() {
  ...
  const items = [
    ...
    },
    {
      ...
      label: "Incubators",
      color: "bg-pink-500",
      link: "/products/laboratory-incubators/laboratory-incubator/bpg-9040a",
      ...
      >
        <div className="flex flex-col gap-4">
          {items.map((item, i) => (
            ...
            >
              {/* <div
                className={`${item.color} w-10 h-10 rounded-full text-white flex items-center justify-center`}
              >
                {item.icon}
              </div> */}
             <div className="flex flex-col gap-5 border border-gray-300 justify-center items-center p-5 rounded-2xl shadow-lg hover:shadow-xl ">
              <img src={item.image} alt={item.label} className="h-full w-full object-cover"/>
               {/* <span className="text-gray-700 font-medium group-hover/item:text-black">
                {item.label}
              </span> */}
                <Link href='/'><div className="flex items-center justify-center gap-3 bg-[#2F3F8D] px-3 py-1 rounded-full w-fit">
