...
export default function Customers() {
    ...
    return (
        <section className="h-screen w-full flex flex-col bg-gradient-to-br from-slate-50 to-blue-50/50">
            ...
            {/* MAIN LAYOUT */}
            <div className="flex-1 grid grid-cols-1 lg:grid-cols-4 gap-0 overflow-hidden p-5">
                {/* ================= MOBILE DROPDOWN ================= */}
                ...
                <div className="hidden lg:block lg:col-span-1 bg-white border-r border-gray-200">
                    <div className="p-6 space-y-2 sticky top-0 h-full flex flex-col">
                        <h3 className="font-semibold text-gray-900 mb-4 text-sm uppercase tracking-wide">Filter by Category</h3>
                        {CATEGORIES.map((cat) => (
                            <button
                                key={cat}
                                onClick={() => setActiveCategory(cat)}
                                className={`w-full text-left px-4 py-3 rounded-lg border transition-all duration-200 text-sm font-medium
                                    ${activeCategory === cat
                                        ? 'bg-[#2B7EC2] text-white border-[#2B7EC2] shadow-md'
