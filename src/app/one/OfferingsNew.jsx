'use client';
import { useState } from 'react';
import { FaArrowLeft, FaArrowRight } from 'react-icons/fa';

// -----------------------------
// PROFILE DATA
// -----------------------------
const profiles = [
    {
        name: "Ovens",
        description: "Reliable and precise oven solutions for industrial and lab applications.",
        link: "/verticals/ovens",
        image: "/about.jpg",
        models: [
            "BOF Series – Mechanical Convection Ovens",
            "BON Series – Natural Convection Ovens",
            "BOV Series – Vacuum Ovens",
        ],
    },
    {
        name: "Incubators",
        description: "Controlled environments for precise incubation of samples.",
        link: "/verticals/incubators",
        image: "/about.jpg",
        models: [
            "BIF Series – Mechanical Convection Incubators",
            "BIT Series – Natural Convection Incubators",
            "BIC Series – Cooling Incubators",
            "BIO Series – CO₂ Incubators",
        ],
    },
    {
        name: "Shakers And Stirrers",
        description: "Efficient mixing and agitation tools for research labs.",
        image: "/about.jpg",
        link: "/verticals/shakers-stirrers",
        models: [
            "BS Series – Orbital Shakers",
            "BIS Series – Incubated Shakers",
            "BS/BIS Series – Shaker Accessories",
            "BMS Series – Square Magnetic Heated Stirrers",
        ],
    },
    {
        name: "Water Baths, Circulators and Chillers",
        description: "Precise temperature control for sample incubation and cooling.",
        image: "/about.jpg",
        link: "/verticals/water-baths-chillers",
        models: [
            "BWB Series – General Purpose Water Bath",
            "BWB Series – Accessories",
            "BPC Series – Heat/Cooling Circulating Bath",
            "BRC Series – Recirculating Chiller",
        ],
    },
    {
        name: "Rotary Evaporators",
        description: "Efficient solvent removal using rotary evaporation technology.",
        image: "/about.jpg",
        link: "/verticals/evaporators",
        models: ["BRE Series – Rotary Evaporators"],
    },
    {
        name: "Pumps",
        description: "Precision vacuum solutions for lab and process needs.",
        image: "/about.jpg",
        link: "/verticals/pumps",
        models: ["V Series – Diaphragm Pumps"],
    },
];

// -----------------------------
// MAIN COMPONENT
// -----------------------------
export default function OfferingsNew() {
    const [startIndex, setStartIndex] = useState(0);

    const nextCards = () => {
        setStartIndex((prev) =>
            prev + 2 >= profiles.length ? 0 : prev + 2
        );
    };

    const prevCards = () => {
        setStartIndex((prev) =>
            prev === 0 ? profiles.length - 2 : prev - 2
        );
    };

    const visibleCards = profiles.slice(startIndex, startIndex + 2);

    return (
        <section className="py-16 px-6">
            <div className="max-w-7xl mx-auto grid lg:grid-cols-3 gap-10 items-center">

                {/* -------------------------------------
             LEFT CONTENT WITH NAV BUTTONS BELOW
        -------------------------------------- */}
                <div className="space-y-6">
                    <div className="text-blue-600 font-semibold flex items-center gap-2 text-sm">
                        <span className="text-[#2B7EC2]">💠</span> Our Products <span className="text-[#2B7EC2]">💠</span>
                    </div>

                    <h2 className="text-4xl font-extrabold leading-tight">
                        Explore Our <br /> Product Categories
                    </h2>

                    <p className="text-gray-600 text-sm leading-relaxed">
                        Discover the full range of scientific equipment with precision and reliability.
                    </p>

                    {/* 🔥 Slider Navigation Buttons Here */}
                    <div className="flex gap-4 pt-4">
                        <button
                            onClick={prevCards}
                            className="bg-[#2B7EC2] text-white px-5 py-2 rounded-full hover:bg-blue-800 transition flex items-center gap-2"
                        >
                            <FaArrowLeft /> Prev
                        </button>

                        <button
                            onClick={nextCards}
                            className="bg-[#2B7EC2] text-white px-5 py-2 rounded-full hover:bg-blue-800 transition flex items-center gap-2"
                        >
                            Next <FaArrowRight />
                        </button>
                    </div>

                </div>

                {/* -------------------------------------
                RIGHT SIDE – 2 CARD GRID
        -------------------------------------- */}
                <div className="lg:col-span-2 grid md:grid-cols-2 gap-6 border p-5 bg-black rounded-2xl">
                    {visibleCards.map((item, index) => (
                        <ModelCard key={index} item={item} />
                    ))}
                </div>

            </div>
        </section>
    );
}

function ModelCard({ item }) {
    return (
        <div className="border border-[#EAEAF4] bg-white p-6 rounded-2xl shadow-sm flex flex-col justify-between h-[550px]">

            {/* TOP SECTION */}
            <div>
                <h3 className="text-xl font-semibold mb-1">{item.name}</h3>
                <p className="text-gray-600 text-sm mb-4">{item.description}</p>

                <img src={item.image} className="rounded-md mb-4 w-full h-[120px] object-cover" />

                {/* MODEL LIST */}
                <div className="bg-gray-50 p-4 rounded-md w-full max-h-[230px] overflow-auto">
                    {item.models.map((model, idx) => (
                        <div key={idx} className="text-gray-700 mb-2 font-medium flex gap-3 items-center">

                            {/* mini spinning icon per model */}
                            <svg
                                className="spin-slow text-[#2B7EC2]"
                                width="20"
                                height="20"
                                viewBox="0 0 30 30"
                                fill="currentColor"
                                xmlns="http://www.w3.org/2000/svg"
                            >
                                <path d="M14.2257 0.947522C14.6258 0.457905 15.3742 0.457905 15.7743 0.947522L16.8781 2.29817C17.181 2.66879 17.704 2.77283 18.1256 2.54633L19.6623 1.72088C20.2193 1.42165 20.9107 1.70806 21.093 2.31352L21.5959 3.98376C21.7339 4.44207 22.1773 4.73834 22.6535 4.69044L24.3891 4.51587C25.0182 4.45258 25.5474 4.98179 25.4841 5.61093L25.3096 7.34647C25.2617 7.8227 25.5579 8.2661 26.0162 8.40409L27.6865 8.90697C28.2919 9.08926 28.5783 9.7807 28.2791 10.3377L27.4537 11.8744C27.2272 12.296 27.3312 12.819 27.7018 13.1219L29.0525 14.2257C29.5421 14.6258 29.5421 15.3742 29.0525 15.7743L27.7018 16.8781C27.3312 17.181 27.2272 17.704 27.4537 18.1256L28.2791 19.6623C28.5783 20.2193 28.2919 20.9107 27.6865 21.093L26.0162 21.5959C25.5579 21.7339 25.2617 22.1773 25.3096 22.6535L25.4841 24.3891C25.5474 25.0182 25.0182 25.5474 24.3891 25.4841L22.6535 25.3096C22.1773 25.2617 21.7339 25.5579 21.5959 26.0162L21.093 27.6865C20.9107 28.2919 20.2193 28.5783 19.6623 28.2791L18.1256 27.4537C17.704 27.2272 17.181 27.3312 16.8781 27.7018L15.7743 29.0525C15.3742 29.5421 14.6258 29.5421 14.2257 29.0525L13.1219 27.7018C12.819 27.3312 12.296 27.2272 11.8744 27.4537L10.3377 28.2791C9.7807 28.5783 9.08926 28.2919 8.90697 27.6865L8.40409 26.0162C8.2661 25.5579 7.8227 25.2617 7.34647 25.3096L5.61093 25.4841C4.98179 25.5474 4.45258 25.0182 4.51587 24.3891L4.69044 22.6535C4.73834 22.1773 4.44207 21.7339 3.98376 21.5959L2.31352 21.093C1.70806 20.9107 1.42165 20.2193 1.72088 19.6623L2.54633 18.1256C2.77283 17.704 2.66879 17.181 2.29817 16.8781L0.947522 15.7743C0.457905 15.3742 0.457905 14.6258 0.947522 14.2257L2.29817 13.1219C2.66879 12.819 2.77283 12.296 2.54633 11.8744L1.72088 10.3377C1.42165 9.7807 1.70806 9.08926 2.31352 8.90697L3.98376 8.40409C4.44207 8.2661 4.73834 7.8227 4.69044 7.34647L4.51587 5.61093C4.45258 4.98179 4.98179 4.45258 5.61093 4.51587L7.34647 4.69044C7.8227 4.73834 8.2661 4.44207 8.40409 3.98376L8.90697 2.31352C9.08926 1.70806 9.7807 1.42165 10.3377 1.72088L11.8744 2.54633C12.296 2.77283 12.819 2.66879 13.1219 2.29817L14.2257 0.947522Z" />
                            </svg>

                            {model}
                        </div>
                    ))}
                </div>
            </div>

            {/* BOTTOM FIXED BUTTON */}
            <div className="flex items-center gap-3 bg-[#2B7EC2] px-6 py-3 rounded-full w-fit mx-auto cursor-pointer mt-4">
                <span className="text-black font-medium text-[16px]">Know More</span>

                <div className="relative w-[30px] h-[30px] text-black">
                    <svg
                        className="spin-slow"
                        width="30"
                        height="30"
                        viewBox="0 0 30 30"
                        fill="currentColor"
                        xmlns="http://www.w3.org/2000/svg"
                    >
                         <path d="M14.2257 0.947522C14.6258 0.457905 15.3742 0.457905 15.7743 0.947522L16.8781 2.29817C17.181 2.66879 17.704 2.77283 18.1256 2.54633L19.6623 1.72088C20.2193 1.42165 20.9107 1.70806 21.093 2.31352L21.5959 3.98376C21.7339 4.44207 22.1773 4.73834 22.6535 4.69044L24.3891 4.51587C25.0182 4.45258 25.5474 4.98179 25.4841 5.61093L25.3096 7.34647C25.2617 7.8227 25.5579 8.2661 26.0162 8.40409L27.6865 8.90697C28.2919 9.08926 28.5783 9.7807 28.2791 10.3377L27.4537 11.8744C27.2272 12.296 27.3312 12.819 27.7018 13.1219L29.0525 14.2257C29.5421 14.6258 29.5421 15.3742 29.0525 15.7743L27.7018 16.8781C27.3312 17.181 27.2272 17.704 27.4537 18.1256L28.2791 19.6623C28.5783 20.2193 28.2919 20.9107 27.6865 21.093L26.0162 21.5959C25.5579 21.7339 25.2617 22.1773 25.3096 22.6535L25.4841 24.3891C25.5474 25.0182 25.0182 25.5474 24.3891 25.4841L22.6535 25.3096C22.1773 25.2617 21.7339 25.5579 21.5959 26.0162L21.093 27.6865C20.9107 28.2919 20.2193 28.5783 19.6623 28.2791L18.1256 27.4537C17.704 27.2272 17.181 27.3312 16.8781 27.7018L15.7743 29.0525C15.3742 29.5421 14.6258 29.5421 14.2257 29.0525L13.1219 27.7018C12.819 27.3312 12.296 27.2272 11.8744 27.4537L10.3377 28.2791C9.7807 28.5783 9.08926 28.2919 8.90697 27.6865L8.40409 26.0162C8.2661 25.5579 7.8227 25.2617 7.34647 25.3096L5.61093 25.4841C4.98179 25.5474 4.45258 25.0182 4.51587 24.3891L4.69044 22.6535C4.73834 22.1773 4.44207 21.7339 3.98376 21.5959L2.31352 21.093C1.70806 20.9107 1.42165 20.2193 1.72088 19.6623L2.54633 18.1256C2.77283 17.704 2.66879 17.181 2.29817 16.8781L0.947522 15.7743C0.457905 15.3742 0.457905 14.6258 0.947522 14.2257L2.29817 13.1219C2.66879 12.819 2.77283 12.296 2.54633 11.8744L1.72088 10.3377C1.42165 9.7807 1.70806 9.08926 2.31352 8.90697L3.98376 8.40409C4.44207 8.2661 4.73834 7.8227 4.69044 7.34647L4.51587 5.61093C4.45258 4.98179 4.98179 4.45258 5.61093 4.51587L7.34647 4.69044C7.8227 4.73834 8.2661 4.44207 8.40409 3.98376L8.90697 2.31352C9.08926 1.70806 9.7807 1.42165 10.3377 1.72088L11.8744 2.54633C12.296 2.77283 12.819 2.66879 13.1219 2.29817L14.2257 0.947522Z" />
                    </svg>

                    <FaArrowRight
                        size={12}
                        className="absolute top-1/2 left-1/2 text-white transform -translate-x-1/2 -translate-y-1/2"
                    />
                </div>
            </div>
        </div>
    );
}


