...
export default function VerticalsNew() {
    return (
        <section className="w-full py-5 bg-gradient-to-br from-slate-50 via-white to-blue-50/50 relative overflow-hidden">
            ...
            <div className="w-full mx-auto  grid grid-cols-1 md:grid-cols-3 gap-8 p-6 relative z-10">
                {profiles.map((profile, index) => (
                    <Link
                        key={index}
                        href={profile.link}
                        className="group bg-white/80 backdrop-blur-xl border-2 border-[#2F4191]/50 rounded-3xl p-8 flex items-start gap-6 hover:border-[#2B7EC2]/70 hover:shadow-2xl hover:shadow-[#2B7EC2]/20 hover:-translate-y-3 hover:rotate-x-1 transition-all duration-500 origin-bottom relative overflow-hidden"
                    >
                        {/* Category Badge */}
